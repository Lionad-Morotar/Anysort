...
const isVoid = (x: SortableValue): boolean => x == undefined
const isVoidType = (x: SortableTypeEnum): boolean => x === 'void'
const getType = (x: SortableValue): SortableTypeEnum => isVoid(x) ? 'void' : Object.prototype.toString.call(x).slice(8, -1).toLowerCase()
const isFn = (x: SortableValue): boolean => getType(x) === 'function'
...
type GetCompareValFn = (x: SortableValue) => ComparableValue
...
type SortFn = (a: SortableValue, b: SortableValue) => SortVal
...
const pass: PassFn = fn => (a, b) => fn(a, b)
const maping: MapingFn = map => fn => (a, b) => fn(map(a), map(b))
const plugin: PluginFn = plug => fn => (a, b) => plug(fn(a, b))

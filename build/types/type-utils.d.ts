...
declare type isEveryCMDValid<T extends unknown[]> = T extends [infer P, ...infer R] ? P extends `${infer Name}()` ? Name extends BuildInPluginNamesWithoutArg ? isEveryCMDValid<R> : false : P extends `${infer Name}(${infer Arg})` ? Name extends BuildInPluginNamesWithArgMaybe ? isEveryCMDValid<R> : false : isEveryCMDValid<R> : true;
